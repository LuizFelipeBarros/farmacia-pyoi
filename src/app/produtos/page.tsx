...
export default function Produtos() {
  ...
  // Extrair categorias únicas do JSON
  const listaCategorias = ["Todos", ...new Set((products as ProdutoProps[]).map(p => p.categoria).filter(Boolean))]
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <header className="mb-10 text-center md:text-left">
          <h1 className="text-3xl font-extrabold text-slate-900 mb-3 tracking-tight">
            Produtos da Farmácia Pyoi
          </h1>
          <p className="max-w-2xl text-slate-600 mx-auto md:mx-0">
            Encontre todos os produtos disponíveis na nossa farmácia, com preços e descrições completas.
          </p>
        </header>
          ...
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
            ...
            <div className="relative flex-1 max-w-md">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-lg text-slate-900 placeholder:text-slate-400 focus:outline-hidden focus:ring-2 focus:ring-purple-500 focus:bg-white transition-all text-sm"
              />
            </div>
            ...
            <div className="flex items-center gap-4 flex-wrap">
              ...
              <select
                ...
                className="bg-slate-50 border border-slate-200 text-slate-950 text-sm rounded-lg focus:ring-purple-500 focus:border-purple-500 p-2 focus:outline-hidden"
              >
                <option value="relevante">Todos</option>
              ...
              {(searchTerm !== "" || categoriaSelecionada !== "Todos" || ordenacaoPreco !== "relevante") && (
                <button
                  ...
                  className="flex items-center gap-2 px-3 py-2 text-xs font-semibold text-slate-600 hover:text-purple-600 border border-slate-200 hover:border-purple-300 hover:bg-purple-50 rounded-lg transition-all cursor-pointer"
                >
        ...
        <section className="w-full">
          {produtosOrdenados.length > 0 ? (
            ...
            <div className="w-full bg-white border border-slate-200 rounded-xl p-12 text-center shadow-xs">
              ...
              <button
                onClick={limparFiltros}
                className="px-4 py-2 bg-purple-600 text-white rounded-lg text-sm font-semibold hover:bg-purple-700 transition-all cursor-pointer"
              >
